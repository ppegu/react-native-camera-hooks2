...
import {
    pausePreview as _pausePreview,
    resumePreview as _resumePreview,
} from "../preview";
...
export const useCamera = (cameraOptions = initialCameraState) => {
    ...
    const [flashMode, { setFlash, toggleFlash }] = useFlash(
        mergedCameraOptions.flashMode
    );
    ...
    return [
        ...
        },
        {
            ...
            takePicture: (options: TakePictureOptions) =>
                _takePicture({ cameraRef }, options),
            recordVideo: (options: RecordOptions) =>
                _recordVideo({ cameraRef }, options),
            stopRecording,
